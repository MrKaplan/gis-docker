...
11. [Site Estático e Blog](#11-site-estatico-e-blog)
...
## 11. Site Estático e Blog

### 11.1. Arquitectura

O site pessoal (portfolio, blog, CV) usa **Hugo** como gerador estático. Não corre na VM - o build é feito via GitHub Actions e os ficheiros HTML/CSS/JS são copiados para o servidor.

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   Repo GitHub   │────▶│ GitHub Actions  │────▶│   VM Oracle     │
│   (Markdown)    │     │  (hugo build)   │     │ (www/ → NGINX)  │
└─────────────────┘     └─────────────────┘     └─────────────────┘
```
...
```
www/
├── landing/              # Site principal Hugo
│   ├── content/          # Conteúdo Markdown
│   │   ├── blog/         # Posts do blog
│   │   ├── portfolio/    # Projectos
│   │   └── about/        # CV / Sobre
│   ├── themes/           # Tema Hugo
│   ├── static/           # Imagens, ficheiros
│   ├── config.toml       # Configuração Hugo
│   └── public/           # OUTPUT (gerado) → servido pelo NGINX
│
└── webgis/               # Aplicações de mapa (Leaflet, etc.)
    ├── hidrologia/
    ├── agricultura/
    └── urbano/
```

### 11.3. Workflow de Publicação

1. Escrever post em Markdown (`content/blog/novo-post.md`)
2. Commit + push para GitHub
3. GitHub Actions executa `hugo build`
4. Ficheiros copiados via SSH/rsync para `/home/ubuntu/gis-docker-stack/www/landing/public/`
5. NGINX serve automaticamente

### 11.4. GitHub Actions (Deploy Hugo)
    ...
    steps:
      ...
      - name: Deploy to Server
        ...
        with:
          ...
          username: ${{ secrets.USERNAME }}
          key: ${{ secrets.SSH_KEY }}
          source: "www/landing/public/*"
          target: "/home/ubuntu/gis-docker-stack/www/landing/"
          strip_components: 3
```

### 11.5. Configuração NGINX

```nginx
# config/nginx/sites-available/default.conf
...
server {
    ...
    # Tiles (proxy para Martin)
    location /tiles {
        proxy_pass http://martin:3000;
        include snippets/proxy-params.conf;
        include snippets/cache-tiles.conf;
    }
}
```

### 11.6. Estrutura de URLs

| URL | Conteúdo | Fonte |
|-----|----------|-------|
| `meudominio.pt/` | Homepage | Hugo |
| `meudominio.pt/blog/` | Blog | Hugo |
| `meudominio.pt/portfolio/` | Projectos | Hugo |
| `meudominio.pt/about/` | CV/Sobre | Hugo |
| `meudominio.pt/webgis/` | Aplicações de mapa | Leaflet/JS |
| `meudominio.pt/api/` | API endpoints | FastAPI |
| `meudominio.pt/tiles/` | Vector tiles | Martin |

### 11.7. Tema Hugo Recomendado

Para portfolio técnico/GIS, considerar:
- **PaperMod** - limpo, rápido, bom para blog
- **Congo** - moderno, suporta portfolio
- **Blowfish** - visual, bom para projectos

Ou criar tema custom para integrar visualmente com o WebGIS.
